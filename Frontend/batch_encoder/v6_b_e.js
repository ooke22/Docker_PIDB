...
async function retrieveProcesses() {
    try {
        ...
        populateProcessCheckboxes(processes);
    } catch (err) {
...
function populateProcessCheckboxes(processes) {
    const container = document.getElementById('wafer_process_container');
    container.innerHTML = '';
    
    processes.forEach(proc => {
        const div = document.createElement('div');
        div.className = 'process-checkbox-item';
        
        const checkbox = document.createElement('input');
        checkbox.type = 'checkbox';
        checkbox.id = `process_${proc.process_id}`;
        checkbox.value = proc.process_id;
        checkbox.addEventListener('change', handleProcessSelection);
        
        const label = document.createElement('label');
        label.htmlFor = `process_${proc.process_id}`;
        label.textContent = `${proc.process_id} - ${proc.description}`;
...
function handleProcessSelection() {
    const selectedCheckboxes = document.querySelectorAll('#wafer_process_container input[type="checkbox"]:checked');
    const timestampWrapper = document.getElementById('timestampContainerWrapper');
    const selectedIds = Array.from(selectedCheckboxes).map(cb => cb.value);

    // Remove timestamp containers for unselected processes
    ...
    // Add timestamp containers for newly selected processes
