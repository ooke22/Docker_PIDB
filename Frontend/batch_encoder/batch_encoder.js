...
function populateProcessDropdown(processes) {
    ...
    dropdown.innerHTML = ''; // Clear exisiting options
...
function handleProcessSelection() {
    ...
    timestampContainerWrapper.innerHTML = '';

    if (selectedOptions.length === 0) {
        timestampContainerWrapper.innerHTML = `<p style="color: gray;">No process selected.</p>`;
    }
...
async function postBatchData() {
    ...
    timestampInputs.forEach(input => {
        ...
        processes.push({ process_id: processId, timestamp: utcTimestamp });
    });
    ...
    try {
        const response = await fetch(testAsyncBatchEncoderAPI, {
            method: 'POST',
            headers: {
